"""Export Software & SaaS taxonomy from migration SQL to CSV + styled Excel.

Mirrors scripts/export-aiml-taxonomy.py — same column shape so the two
files can be diffed/merged downstream. The only structural difference is
that the software-saas migration SQL carries a trailing color literal
(e.g. `, '#3B82F6'`) before the FROM clause, so the regex below tolerates
an optional final literal.

Levels:
  L1 = Software & SaaS (root)
  L2 = 37 sub-categories  (migration-software-saas-taxonomy.sql)
  L3 = 1783 sub-sub-categories (same file)
  L4 = ~1770 listing types (migration-remap-software-saas-listing-types.sql)
       — each listing type uses 'Group: Specific' naming, which we split
         the same way as the AI/ML export.
"""
import re
import csv
import sys
...
SQL_CATS = ROOT / "database" / "migration-software-saas-taxonomy.sql"
SQL_LT = ROOT / "database" / "migration-remap-software-saas-listing-types.sql"
OUT_CSV = ROOT / "exports" / "software-saas-taxonomy-full.csv"
OUT_XLSX = ROOT / "exports" / "software-saas-taxonomy-full.xlsx"
...
# The trailing `, '#3B82F6'` color is optional so the same regex also
# matches sectors that omit the color literal.
pat = re.compile(
...
# Sheet 1: Flat table
ws = wb.active
ws.title = "Software-SaaS Taxonomy"
...
# Header style — same look as the AI/ML export so the two workbooks
# feel like siblings.

#!/usr/bin/env python3
"""
Remap 1,660 Local Business listing types from old L3 categories to new L3 categories.
Reads: existing listing types from MySQL DB + new L3s from Excel
Writes: database/migration-remap-local-business-listing-types.sql
"""
...
EXCEL = r"C:\Users\AADIL PARMAR\Downloads\Local-Business-Taxonomy-900plus.xlsx"
SHEET = "Local Business Navigation"
L1_SLUG = "local-business"
OUT_DIR = os.path.join(os.path.dirname(__file__), "..", "database")
OUT_FILE = os.path.join(OUT_DIR, "migration-remap-local-business-listing-types.sql")
...
# ─── Manual overrides: (old_l2, old_l3) -> new_l3_slug ────────────
MANUAL_OVERRIDES = {
    # Health specialists with no direct L3 match
    ('Health & Wellness', 'Chiropractic'): 'holistic-health-centers',
    ('Health & Wellness', 'Hearing'): 'audiologists',
    ('Health & Wellness', 'Optician'): 'ophthalmologists',
    ('Health & Wellness', 'Prenatal & Postnatal'): 'ob-gyns',
    ('Health & Wellness', 'Pharmacy'): 'family-medicine-doctors',
    ('Health & Wellness', 'Fitness'): 'yoga-studios',
    # Video services → Wedding Videographers (closest video production L3)
    ('Events & Entertainment', 'Videography'): 'wedding-videographers',
    # Consumer electronics → Hobby Shops (closest electronics/gadget retail)
    ('Retail & Shopping', 'Electronics'): 'hobby-shops',
    # Seasonal retail
    ('Retail & Shopping', 'Seasonal'): 'holiday-markets',
    # Arts & crafts lessons → Art Supply Stores (closest creative L3)
    ('Education & Childcare', 'Lessons & Classes'): 'art-supply-stores',
}
...
SYNONYMS = {
    ...
    'automation': 'automat', 'automated': 'automat',
    'scheduling': 'schedul', 'scheduler': 'schedul',
    'consulting': 'consult', 'consultant': 'consult',
    'analytics': 'analytic', 'analysis': 'analytic',
    'optimization': 'optim', 'optimizer': 'optim',
    'security': 'secur', 'securing': 'secur', 'secure': 'secur',
    'marketing': 'market', 'marketers': 'market',
    'designing': 'design', 'designers': 'design',
    'photography': 'photo', 'photographer': 'photo', 'photographers': 'photo',
    'installation': 'install', 'installer': 'install', 'installers': 'install',
    ...
    'cleaning': 'clean', 'cleaners': 'clean', 'cleaner': 'clean',
    'repair': 'repair', 'repairs': 'repair', 'repairing': 'repair',
    'training': 'train', 'trainer': 'train', 'trainers': 'train',
    'tutoring': 'tutor', 'tutors': 'tutor',
    'grooming': 'groom', 'groomers': 'groom', 'groomer': 'groom',
    'painting': 'paint', 'painters': 'paint', 'painter': 'paint',
    'plumbing': 'plumb', 'plumbers': 'plumb', 'plumber': 'plumb',
    'roofing': 'roof', 'roofers': 'roof', 'roofer': 'roof',
    'flooring': 'floor',
    'landscaping': 'landscap', 'landscapers': 'landscap', 'landscaper': 'landscap',
    'catering': 'cater', 'caterers': 'cater', 'caterer': 'cater',
    'veterinary': 'vet', 'veterinarian': 'vet', 'veterinarians': 'vet',
    'salons': 'salon',
    'dentists': 'dentist',
    'therapists': 'therapist',
    'counseling': 'counsel', 'counselors': 'counsel', 'counselor': 'counsel',
...
worst = sorted([(s, lt) for s, lt in zip(match_scores, old_listing_types) if s >= 0], key=lambda x: x[0])[:20]
print(f"\nLowest-scoring matches (bottom 20):")
...
sql_lines = []
sql_lines.append("-- =================================================================")
sql_lines.append("-- Local Business Listing Type Remapping")
sql_lines.append(f"-- {len(old_listing_types)} listing types -> {len(assignments)} new L3 categories")
sql_lines.append("-- Generated by remap-local-business-listing-types.py")
sql_lines.append("-- =================================================================")
sql_lines.append("")
sql_lines.append("-- Run AFTER migration-local-business-taxonomy.sql")
sql_lines.append("")

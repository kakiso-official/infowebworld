/**
 * Read-only dump of every submission under local-businesses.
 *   node scripts/inspect-local-businesses-listings.mjs
 * Writes exports/current-local-businesses-listings.json
 */
...
const [[root]] = await conn.execute(
  `SELECT id, name, slug FROM categories WHERE slug = 'local-businesses' AND level = 1 LIMIT 1`
)
if (!root) throw new Error('local-businesses L1 not found')
console.log(`local-businesses L1: id=${root.id}`)
...
writeFileSync('exports/current-local-businesses-listings.json', JSON.stringify({
  generated_at: new Date().toISOString(),
  local_businesses_root: { id: root.id, name: root.name, slug: root.slug },
...
console.log('Wrote exports/current-local-businesses-listings.json')
await conn.end()

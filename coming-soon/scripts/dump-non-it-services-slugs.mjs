/**
 * Dump every category slug that is NOT under it-services-agencies — for the
 * Python migration generator's collision check.
 *
 *   node scripts/dump-non-it-services-slugs.mjs
 *
 * Writes exports/non-it-services-slugs.json
 */
...
const [[root]] = await conn.execute(
  `SELECT id FROM categories WHERE slug = 'it-services-agencies' AND level = 1 LIMIT 1`
)
...
writeFileSync('exports/non-it-services-slugs.json', JSON.stringify({
  generated_at: new Date().toISOString(),
  it_services_descendant_count: ids.size,
  non_it_services_slugs: nonIt,
}, null, 2), 'utf8')
console.log(`Wrote exports/non-it-services-slugs.json — ${nonIt.length} slugs (it-services-agencies descendants excluded: ${ids.size})`)
await conn.end()

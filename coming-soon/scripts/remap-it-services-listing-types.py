#!/usr/bin/env python3
"""
Remap 780 IT Services & Agencies listing types from old L3 categories to new L3 categories.
Reads: existing listing types from MySQL DB + new L3s from Excel
Writes: database/migration-remap-it-services-listing-types.sql
"""
...
OUT_FILE = os.path.join(OUT_DIR, "migration-remap-it-services-listing-types.sql")
...
MANUAL_OVERRIDES = {
    # Vendor Advisory → IT Vendor Management (not power tool repair!)
    ('IT Consulting', 'Vendor Advisory'): 'it-vendor-management-services',
    # IoT Development → Embedded Software Development (closest hardware-adjacent dev)
    ('Software Development', 'Emerging Technologies'): 'embedded-software-development',
}
...
SYNONYMS = {
    ...
    'production': 'produc', 'producing': 'produc', 'producer': 'produc',
...
sql_lines = []
sql_lines.append("-- =================================================================")
sql_lines.append("-- IT Services & Agencies Listing Type Remapping")
sql_lines.append(f"-- {len(old_listing_types)} listing types -> {len(assignments)} new L3 categories")
sql_lines.append("-- Generated by remap-it-services-listing-types.py")
sql_lines.append("-- =================================================================")
sql_lines.append("")
sql_lines.append("-- Run AFTER migration-it-services-taxonomy.sql")
sql_lines.append("")

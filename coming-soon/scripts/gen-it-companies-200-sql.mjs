...
import { EMAILS_RAW } from './it-companies-200.emails.mjs'

/* Enrichment overlay, keyed by normalised domain. */
const ENRICH = new Map(
  ENRICH_RAW.map((e) => [String(e.domain || '').toLowerCase().replace(/^www\./, ''), e]),
)
/* Real published contact emails (overlay); falls back to info@domain when absent. */
const EMAIL = new Map(
  EMAILS_RAW.filter((e) => e.email).map((e) => [String(e.domain || '').toLowerCase().replace(/^www\./, ''), e.email]),
)
...
function insert(c) {
  ...
  const vals = [
    'UUID()', s(c.company_name), s(c._slug), s(`${c.company_name} Team`), s(EMAIL.get(c._dom) || `info@${c._dom}`), s(ctry.ph), 'NULL', s(c.website),
...
const emailUpdates = COMPANIES
  .filter((c) => EMAIL.get(c._dom))
  .map((c) => `UPDATE submissions SET email='${String(EMAIL.get(c._dom)).replace(/'/g, "''")}' WHERE slug='${c._slug}' AND listing_mode='company';`)
const emailSql = `-- Real, web-verified contact emails for the live 200 IT company listings.
-- Replaces the info@<domain> placeholder for the ${emailUpdates.length} companies where a real
-- published email was found; the other ${COMPANIES.length - emailUpdates.length} keep info@<domain>
...
${emailUpdates.join('\n')}
`
writeFileSync(new URL('../database/update-it-companies-emails.sql', import.meta.url), emailSql)
...
console.log(`Real emails: ${emailUpdates.length}/200 (rest keep info@domain) -> database/update-it-companies-emails.sql`)

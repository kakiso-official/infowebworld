...
for (const c of COMBINED) {
  ...
  /* Reject, do NOT mutate — see note in gen-companies-all.mjs. */
  if (seenSlug.has(slug)) { skippedDupSlug++; continue }
...
/* ── Countries these listings reference. The base `countries` table shipped
   with only ~10 rows, so any HQ outside them resolved via COALESCE(...) to the
   US fallback id and rendered "United States". INSERT IGNORE adds the missing
   ones (existing codes untouched) so the FK always resolves to the real country.
   Names MUST match what hq_location stores (both come from this COUNTRY map). ── */
const countryValues = Object.entries(COUNTRY)
  .map(([code, c], i) => `('${String(c.name).replace(/'/g, "''")}', '${code}', '${c.ph}', ${210 + i})`)
const countriesSql =
...
-- Ensure every country referenced below exists, so country_id resolves to the
-- real country instead of silently falling back to the US id. Existing untouched.
${countriesSql}
...
/* ── companion: GLOBAL country_id repair for already-loaded company rows ──
   Fixes the "shows United States" bug across ALL seeded company listings (the
   200 + the 259 + the 9), not just this batch: add every country, then set
   country_id from the real country name already stored at the end of
   hq_location. Scoped to user_id IS NULL so real users' listings are never
   touched. Idempotent. Run in phpMyAdmin, then redeploy (profiles are SSG). */
const fixSql = `-- ============================================================
...
`
writeFileSync(new URL('../database/fix-listing-countries.sql', import.meta.url), fixSql)

/**
 * Read-only dump of every submission under professional-services.
 *   node scripts/inspect-professional-services-listings.mjs
 */
...
const [[root]] = await conn.execute(`SELECT id, name, slug FROM categories WHERE slug = 'professional-services' AND level = 1 LIMIT 1`)
if (!root) throw new Error('professional-services L1 not found')
console.log(`professional-services L1: id=${root.id}`)
...
const [subs] = await conn.execute(`
  SELECT s.id, s.uuid, s.company_name, s.tagline, s.description, s.status,
         ...
         p.name AS l2_name, gp.name AS l1_name, lt.name AS lt_name
    FROM submissions s
...
writeFileSync('exports/current-professional-services-listings.json', JSON.stringify({
  generated_at: new Date().toISOString(),
  professional_services_root: { id: root.id, name: root.name, slug: root.slug },
  ...
  submissions: subs.map(r => ({
    ...
    description: r.description ? String(r.description).slice(0, 400) : null, status: r.status,
    ...
    old_cat_slug: r.cat_slug, old_cat_level: r.cat_level, old_listing_type: r.lt_name || null,
  })),
}, null, 2), 'utf8')
console.log('Wrote exports/current-professional-services-listings.json')
await conn.end()

            ...
            {searchResults && searchResults.length > 0 ? (
              <div className="cb-cats">
                {searchResults.map(cat => {
                  ...
                  const lvlPastel: Record<number, string> = { 1: '#FECACA', 2: '#BFDBFE', 3: '#E9D5FF' }
                  ...
                  return (
                    <div key={cat.id} className="cb-cat" style={{ '--sc-pastel': lvlPastel[cat.level], '--sc': lvlColor[cat.level] } as React.CSSProperties}>
                      <Link href={`/category/${cat.slug}`} className="cb-cat-hd">
                        <h3 className="cb-cat-name">{cat.name}</h3>
                      </Link>
                      {trail && (
                        <div className="cb-cat-body">
                          <span className="cb-cat-trail">
                            <span className="cb-cat-trail-lvl" style={{ color: lvlColor[cat.level] }}>{lvlLabel[cat.level]}</span>
                            {trail}
                          </span>
                        </div>
                      )}

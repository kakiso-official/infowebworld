...
/**
 * Load a submission and build the dashboard form's `initialFormState`. Shared
 * by the owner edit page (/dashboard/listings/[uuid]/edit) and the admin edit
 * page (/iww-hq/submissions/[id]/edit).
 *
 * Pass `ownerId` to constrain to that user (owner flow). Omit it for admin —
 * any listing loads by numeric id or uuid.
 */
export async function loadEditData(
  ...
  const whereCol = isUuid ? 's.uuid = ?' : 's.id = ?'
  const ownerClause = opts?.ownerId != null ? ' AND s.user_id = ?' : ''
  const params: (string | number)[] = [idOrUuid]
  if (opts?.ownerId != null) params.push(opts.ownerId)

...
/* ═══════════════════════════════════════════════════════════════════════
   ...
   Same .tlp-* classes as the test-landing-page hero, but every /api/search
   call passes `sector=<this-sector>` so the dropdown surfaces only
   sector-scoped categories, listings, and blog hits.
   ═══════════════════════════════════════════════════════════════════════ */
...
export default function HeroSearch({ sectorSlug, title, sub, placeholder, avgRating = 0, totalReviews = 0, totalListings = 0 }: Props) {
  ...
  const fetchResults = useCallback(async (term: string) => {
    ...
    setLoading(true)
    try {
      const r = await fetch(`/api/search?q=${encodeURIComponent(term)}&sector=${sectorSlug}`)

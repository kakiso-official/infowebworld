...
const btn = (active: boolean): React.CSSProperties => ({ padding: '.25rem .65rem', borderRadius: 999, border: '1.5px solid var(--h-border)', fontSize: '.62rem', fontWeight: 700, cursor: 'pointer', fontFamily: "var(--font-nunito)", background: active ? '#E8553D' : '#fff', color: active ? '#fff' : 'var(--h-muted)', transition: 'all .2s' })
...
export default function BlogEditor({ post, onChange, onSave, onCancel }: Props) {
  ...
  const set = useCallback(<K extends keyof BlogPost>(k: K, v: BlogPost[K]) => {
    const updated = { ...post, [k]: v }
    /* Auto-calc read time */
    if (k === 'body') updated.readTime = Math.max(1, Math.ceil(((v as string)?.split(/\s+/).length || 0) / 200))
    onChange(updated)
  }, [post, onChange])

  const insertMd = (md: string, wrap: boolean) => {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.15rem' }}>
      ...
      <div>
        ...
        <p style={{ fontSize: '.6rem', color: 'var(--h-muted)', marginTop: '.3rem' }}>
          Slug: <span style={{ color: 'var(--h-heading)', fontWeight: 600 }}>/blog/post?slug={slugPreview}</span>
        </p>
      </div>
      ...
      <div>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '.4rem' }}>
          ...
          <button type="button" onClick={() => setShowCovers(!showCovers)} style={btn(showCovers)}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(100px, 1fr))', gap: '.4rem', marginBottom: '.65rem', padding: '.65rem', borderRadius: 14, border: '1.5px solid var(--h-border)', background: 'var(--h-bg)' }}>
            {stockCovers.map(c => (
              <button key={c.url} type="button" onClick={() => { set('coverImage', c.url); setShowCovers(false) }}
                ...
                <img src={c.url} alt={c.label} style={{ width: '100%', aspectRatio: '16/10', objectFit: 'cover', display: 'block' }} />
        ...
        <div style={{ display: 'flex', gap: '.5rem' }}>
          ...
          <button type="button" onClick={() => { if (customUrl) { set('coverImage', customUrl); setCustomUrl('') } }} style={{ ...btn(false), height: 44, padding: '0 1rem', borderColor: 'var(--h-border)' }} disabled={!customUrl}>Apply</button>
        </div>
      ...
      <div>
        ...
        <textarea style={{ ...inp, height: 'auto', padding: '.65rem .85rem', resize: 'vertical', minHeight: 64, lineHeight: 1.6 }} placeholder="A brief summary (1-2 sentences) — shown on listing cards..." rows={2} maxLength={200} value={post.excerpt} onChange={e => set('excerpt', e.target.value)} onFocus={fo} onBlur={bl} />
      </div>
      ...
      <div>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '.5rem' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '.65rem' }}>
            <span style={{ fontSize: '.58rem', color: 'var(--h-muted)', fontWeight: 600 }}>{wordCount} words &middot; {post.readTime} min read &middot; {charCount} chars</span>
            <div style={{ display: 'flex', gap: '.25rem' }}>
              {(['write', 'preview'] as const).map(t => (
                <button key={t} type="button" onClick={() => setTab(t)} style={btn(tab === t)}>{t === 'write' ? 'Write' : 'Preview'}</button>
              ))}
            </div>
          </div>
        </div>
        ...
        {tab === 'write' && (
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '.25rem', marginBottom: '.4rem' }}>
            {toolbar.map(t => (
              <button key={t.l} type="button" onClick={() => insertMd(t.md, t.w)}
                style={{ padding: '.2rem .5rem', borderRadius: 8, border: '1.5px solid var(--h-border)', fontSize: '.6rem', fontWeight: 700, cursor: 'pointer', background: '#fff', color: 'var(--h-heading)', fontFamily: 'monospace', transition: 'all .15s' }}>
                {t.l}
              </button>
            ))}
          </div>
        ...
        {tab === 'write' ? (
          <textarea ref={bodyRef} style={{ ...inp, height: 'auto', padding: '.85rem', resize: 'vertical', minHeight: 360, lineHeight: 1.7, fontFamily: "'SF Mono', 'Fira Code', 'Consolas', monospace", fontSize: '.8rem' }} placeholder={'Write your blog post in Markdown...\n\n## Getting Started\n\nYour content here...\n\n> Blockquotes look like this\n\n**Bold** and _italic_ text'} value={post.body} onChange={e => set('body', e.target.value)} onFocus={fo} onBlur={bl} />
        ) : (
          <div style={{ minHeight: 360, padding: '1.5rem', borderRadius: 14, border: '1.5px solid var(--h-border)', background: '#fff', overflowWrap: 'break-word' }}>
            {post.body ? <div className="blog-body" dangerouslySetInnerHTML={{ __html: html }} /> : <p style={{ color: 'var(--h-muted)', fontStyle: 'italic', fontSize: '.85rem' }}>Nothing to preview yet. Switch to Write tab and start typing...</p>}
          </div>
        )}
      </div>
      ...
      <div style={{ display: 'flex', gap: '.65rem', paddingTop: '1rem', borderTop: '1px solid var(--h-border-light)', flexWrap: 'wrap' }}>
        ...
        <button type="button" onClick={() => onSave('published')} disabled={!post.title || !post.excerpt || !post.body}
          style={{ padding: '.55rem 1.25rem', borderRadius: 999, border: 'none', background: '#E8553D', color: '#fff', fontSize: '.78rem', fontWeight: 700, cursor: !post.title || !post.excerpt || !post.body ? 'not-allowed' : 'pointer', opacity: !post.title || !post.excerpt || !post.body ? .4 : 1, fontFamily: "var(--font-nunito)", transition: 'all .3s' }}>Publish</button>
      </div>
    </div>
  )
}

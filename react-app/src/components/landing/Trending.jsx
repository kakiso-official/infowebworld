...
const items = [
  { rank:1, biz:'cloudsync-pro', name:'CloudSync Pro', cat:'Technology', rating:'4.9', votes:'847', change:'+32%', iconBg:'rgba(108,114,241,.1)', color:'var(--accent)', cls:'trend-item--gold', icon:<path d="M18 10h-1.26A8 8 0 1 0 9 20h9a5 5 0 0 0 0-10z" /> },
  { rank:2, biz:'the-garden-table', name:'The Garden Table', cat:'Restaurant', rating:'4.9', votes:'723', change:'+28%', iconBg:'rgba(239,107,74,.1)', color:'var(--coral)', cls:'trend-item--silver', icon:<><path d="M3 2v7c0 1.1.9 2 2 2h4a2 2 0 0 0 2-2V2" /><path d="M7 2v20" /><path d="M21 15V2a5 5 0 0 0-5 5v6c0 1.1.9 2 2 2h3zm0 0v7" /></> },
  { rank:3, biz:'mindbridge-wellness', name:'MindBridge Wellness', cat:'Healthcare', rating:'4.8', votes:'691', change:'+24%', iconBg:'rgba(139,92,246,.1)', color:'var(--plum)', cls:'trend-item--bronze', icon:<path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z" /> },
      ...
      <div className="container">
        ...
        <div className="trend-grid">
          {[col1,col2].map((col,ci) => (
            <div className="trend-col" key={ci}>
              {col.map(t => (
                <Link to={`/listing?biz=${t.biz}`} className={`trend-item${t.cls ? ' '+t.cls : ''}`} key={t.rank}>
                  <span className="trend-item-rank">{t.rank}</span>
                  <div className="trend-item-icon" style={{background:t.iconBg}}><svg viewBox="0 0 24 24" stroke={t.color} fill="none" strokeWidth="1.5">{t.icon}</svg></div>
                  <div className="trend-item-body">
                    <div className="trend-item-top"><span className="trend-item-name">{t.name}</span><span className="trend-item-cat">{t.cat}</span></div>
                    <div className="trend-item-bottom"><div className="trend-item-rating"><StarSvg /><span>{t.rating}</span></div><span className="trend-item-votes">{t.votes} votes</span></div>
                  </div>
                  <div className="trend-item-change trend-item-change--up">{t.change}</div>
                </Link>
              ))}
            </div>
          ))}
